...
A new mechanism was added after this brief was written and has never been reviewed: a project developer applying for catalyst-fund financing pays a registration fee, sent to the same management wallet discussed in Section 6 above — initially set at 1,000 ACT, raised to **5,000 ACT** the same day (7 Sept 2026). See `MANAGEMENT_FEE.md`, "Project registration fee," and `PROTOCOL.md` §8 step 1 for the full mechanism, including the actual payment flow now built into `apply.html`.
