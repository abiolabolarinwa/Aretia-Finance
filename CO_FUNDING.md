...
**Status:** ✅ Address derived and verified, 4 Sept 2026. Not yet funded — no contributions have been solicited or received.
...
| Address | `27e1P7arTqPVdKNBEmQhtrjMXnVycrbuqkEHoCVdZSPY` |
| Custody | Squads vault index 1, under the same multisig as the treasury (`AF8qvhgkZJJE6ascFN4MAwWSEGKpyi6oW6Ht9CySgkmX`) — same 2-of-3 signer set, same governance, no new keys or signers. |
| Relationship to main treasury | Vault index 0 (`3FyoJdvC7FaZDEt5YoLHF3PB2xo3vtp4GWTBTN6cyzZg`) holds ACT supply and protocol-fee-derived catalyst fund capital. Vault 1 holds only externally contributed capital. Both are controlled by the same multisig, so disbursement from either still requires 2-of-3 approval. |
| Network | Solana Mainnet |
| Current balance | 0 |

## How the address was derived

Squads V4 vaults are deterministic PDAs derived from the multisig address and a vault index — they don't require a separate "creation" transaction to exist as an address; they become active the moment they're referenced in a transaction or receive funds. The derivation was computed offline (no RPC call needed, just the `@sqds/multisig` SDK's `getVaultPda`) and cross-checked: deriving vault index 0 with the same code reproduces `3FyoJdvC7FaZDEt5YoLHF3PB2xo3vtp4GWTBTN6cyzZg` exactly, the same address already confirmed live in `TREASURY.md` and `MINT.md`. That match is what makes vault index 1's address trustworthy without needing to execute anything on-chain first. See `scripts/management-fee-proposal/derive-vault.mjs`.
