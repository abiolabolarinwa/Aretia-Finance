...
2. ~~**DEX/AMM Token-2022 compatibility**~~ — **Resolved 18 Sept 2026.** Researched, then proven with a real devnet swap. All three major Solana AMMs support `TransferFeeConfig` natively, no whitelist/badge needed:
   ...
   - **Orca proven too (18 Sept 2026)**: created a real Orca Whirlpool "splash pool" (`3kY8x7mnxKKcUHmDnfLJ1M8Yv6G8NQ7K8LKkbEb8C4Eo`, on Orca's real devnet deployment -- the Whirlpool program is deployed at the *same* address on devnet and mainnet, `whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc`, confirmed live via `solana account`; the `WhirlpoolsConfig` account is devnet-specific, `FcrweFY1G9HJAHG5inkGB6pKg1HZ6x9UC2WioAfWrGkR`, also confirmed live) pairing the same test-ACT against test-USDC, opened a full-range position, and swapped. Confirmed from the transaction's own balances: the implied fee on the ACT leg landed at **exactly 350 bps**, same as Raydium's result. Used the *legacy* `@orca-so/whirlpools-sdk` (0.22.x), not the newer `@orca-so/whirlpools` package -- the newer one requires Solana Web3.js v2, a different major version incompatible with the v1.99.0 this whole project uses; the legacy SDK's peer range is plain `@solana/web3.js ^1.98.4`. Hit and fixed one real SDK quirk along the way: `increaseLiquidityQuoteByInputToken`'s computed `minSqrtPrice`/`maxSqrtPrice` bounds came back too narrow for this token-extensions pool (rejected with `PriceSlippageOutOfBounds` even at 50% nominal slippage, traced to the exact on-chain check by reading Orca's own source), fixed by widening to the SDK's own protocol-wide `MIN_SQRT_PRICE_BN`/`MAX_SQRT_PRICE_BN` constants for this mechanism test. See `scripts/exercise-devnet-orca-whirlpool.mjs`.
   - **Meteora proven too (18 Sept 2026)**: created a real Meteora DAMM v2 (constant-product) pool (`GobTxZ9CqYpDTBVKxCpZK5iEob7WxBucybyt7Q3PHBBv`, via `createCustomPool` on Meteora's real devnet deployment -- the CP-AMM program is deployed at the *same* address on devnet and mainnet, `cpamdpZCGKUy5JxQXB4dcpGPiikHawvSWAd6mEn1sGG`, confirmed live via `solana account`) pairing the same test-ACT against test-USDC, seeded a full-range position, and swapped. The implied fee landed at **exactly 350 bps** again -- the third independent AMM to confirm this to the basis point. Used `@meteora-ag/cp-amm-sdk` (DAMM v2), not `@meteora-ag/dlmm` -- DLMM's bin-based liquidity model needs considerably more setup (bin arrays, bitmap extensions) for the same proof, where DAMM v2 is a plain constant-product AMM, the closest match to Raydium CPMM's shape. Hit and fixed one real SDK quirk: `getInitialCompoundingPoolInformation`'s own `sqrtMinPrice`/`sqrtMaxPrice` are sentinel values (`0`/`u128::MAX`, meaning "unbounded" for a *position*), not valid *pool-level* range bounds -- passing them directly into `createCustomPool` failed with `InvalidPriceRangeError`; fixed by using the SDK's own protocol-wide `MIN_SQRT_PRICE`/`MAX_SQRT_PRICE` constants for the pool's range instead. See `scripts/exercise-devnet-meteora-damm.mjs`.
   - **Still open**: none of the three AMMs has been tried against the real ACT mint (that's a mainnet action requiring separate approval, not attempted here). AMM v4 remains confirmed unusable for Token-2022 regardless.
3. ~~**SOL payment support**~~ — **Resolved 18 Sept 2026.** `buy_with_sol` is implemented for real (no longer a stub) and proven against a real, live Pyth price update on devnet:
   ...
   - **`refund_sol`'s success path — resolved 18 Sept 2026.** A third throwaway deployment (`6zWExLaMGuB2ywg6VickQnML2rb2ViSXbmyfdkbH9WKV`, closed afterward) bought 0.02 SOL against a real live price update, let the sale window close under a soft cap deliberately set high enough to miss, `finalize`d to `Refunding`, then called `refund_sol` -- it returned exactly the 20,000,000 lamports contributed, and a second call was correctly rejected with `AlreadyRefunded`. 3/3 checks passed; see `scripts/exercise-devnet-refund-sol.mjs`. Still only devnet-test amounts have been exercised (0.02-0.05 SOL).
