...
5. **Full devnet exercise** — **Partially done (18 Sept 2026):** buy (both currencies) → finalize (hard-cap-reached, soft-cap-met happy path, sweeping to the real Squads treasury ATAs) → claim (TGE portion + partial linear vesting) all exercised and passing. **Still not exercised:** the soft-cap-missed → `refund` path (needs a second presale instance, since `presale_config` is a per-deployment singleton) and claiming across multiple vesting checkpoints over real elapsed days (this devnet build deliberately was not compiled with `test-fast-clock`).
