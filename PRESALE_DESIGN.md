...
2. ~~**DEX/AMM Token-2022 compatibility**~~ — **Researched 18 Sept 2026 (not yet tested against the real ACT mint on a real pool).** All three major Solana AMMs support `TransferFeeConfig` natively, no whitelist/badge needed:
   ...
   - **Still needed before mainnet**: pick one (Raydium CPMM is the most explicitly documented and most liquid), and do a real devnet-or-mainnet-fork swap/deposit/withdraw test against the actual pool program with a transfer-fee Token-2022 mint before trusting it with the real ACT mint and real liquidity.
