...
5. **Full devnet exercise** — **Done, except one item (18 Sept 2026):** happy path (buy both currencies → finalize on hard-cap-reached → sweeps to the real Squads treasury ATAs → claim TGE + partial linear vesting) and the soft-cap-missed path (buy → finalize → `Refunding` → `refund` returns exactly what was put in → a second refund attempt correctly rejected with `AlreadyRefunded`) are both exercised and passing. The refund path needed a second, throwaway program deployment (`5zT4oG1qBMPjdvA6QZ16pX12eFjbH1znUtvUEdD8UCe8` -- same source, temporarily built with a different `declare_id!` since Anchor enforces that the invoked program's on-chain address matches the ID baked into the binary; reverted back to the real ID afterward, see `scripts/exercise-devnet-refund.mjs`), since `presale_config` is a per-deployment singleton PDA and the primary devnet instance was already `Finalized`. **Still not exercised:** claiming across multiple vesting checkpoints over real elapsed days (this devnet build deliberately was not compiled with `test-fast-clock`).
