...
**Status (updated 12 Sept 2026):** Program **compiles clean** and is **deployed and live on devnet** at `DpaKPgqdcoY2pQFrHc5xgP4eegbnaCWuRVYFMrrzThtH`, confirmed independently via `solana program show` (owner `BPFLoaderUpgradeab1e11111111111111111111111`, data length exactly matching the local `.so`). Not deployed to mainnet, and no instruction has been called yet — the deploy proves the bytecode loads and is executable, not that `initialize_config`/`stake`/`unstake` behave correctly. See "Build log" below, and don't treat "it's on devnet" as "it's been tested"; those are different claims.
...
- **12 Sept 2026** — Devnet deployment **succeeded**, in two attempts: the first `solana program deploy` failed partway ("41 write transactions failed", again the environment's flaky network dropping some of the many small chunk-upload transactions a ~287 KB program requires) but left a resumable buffer account; resuming with `--buffer <printed-buffer-address>` (the deployer key already held write authority over it, no need to recover the ephemeral buffer keypair the CLI printed a seed phrase for) completed the deploy. Confirmed independently, not just trusted from the CLI's own success message, via `solana program show DpaKPgqdcoY2pQFrHc5xgP4eegbnaCWuRVYFMrrzThtH --url devnet`:
  ...
  ```
  **Upgrade authority currently sits on the throwaway devnet deployer key above**, not the treasury multisig. That's fine for devnet iteration but must be transferred (`solana program set-upgrade-authority`) to a real, intentional holder, matching how mint/fee authorities were handled, before this deployment is treated as anything more than a scratch environment — and is a hard requirement, not a nice-to-have, before any mainnet deploy uses this same pattern.

## Still open

- [ ] **No instruction has been called yet.** The deploy proves the bytecode loads, is owned by the correct loader, and is executable, full stop. It does not prove `initialize_config`, `stake`, or `unstake` behave correctly, or even that they don't immediately fail on `try_accounts` -- that requires actually sending a transaction against them, which hasn't happened.
- [ ] **No test suite has run.** `tests/act-staking.ts` is a hand-written skeleton, never executed against a local validator or the devnet deployment above -- `anchor test` needs the Anchor CLI (not installed) or an equivalent manual `solana-test-validator`/devnet + TS client setup. This is a materially weaker verification state than "it's deployed" and should not be conflated with it.
- [ ] **Devnet program's upgrade authority is a throwaway key**, not yet transferred to anything intentional (see build log above).
