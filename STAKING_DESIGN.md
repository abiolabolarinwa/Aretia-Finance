...
**Status (updated 13 Sept 2026):** Program **compiles clean**, is **deployed and live on devnet**, and `initialize_config`, `stake` (fresh + top-up), and `unstake`'s rejection of early withdrawal have all been **called for real against the live deployment and verified correct** — 13/13 checks passed on the stake/unstake run alone. Testing `stake` caught a real bug: the tier thresholds passed to `initialize_config` were unscaled (whole-token numbers where the program expects base units), silently maxing every staker straight to tier 4 regardless of size. Fixed on-chain via `update_tier_config` and fixed in the setup script so it doesn't recur. **Not yet tested: `unstake`'s success path** (after a lock actually matures) — the shortest configured lock is 30 real days, devnet has no clock-warping, and a local-validator route (which would allow a shrunk test clock) was attempted and blocked by what looks like a Windows security product intercepting `solana-test-validator`'s genesis-archive unpacking, confirmed independent of admin privilege. Not deployed to mainnet. See "Build log" below for exactly what's covered and what isn't.
...
- **13 Sept 2026** — `stake` and `unstake` **called for real against the devnet deployment**, via `scripts/stake-unstake-devnet-test.mjs`, reusing the same config/test-mint `initialize_config` set up (the config PDA is a singleton, so a second mint isn't possible without redeploying). 13/13 checks passed: a fresh `stake` correctly transfers tokens and computes CAS/tier; a top-up `stake` correctly sums the amount and keeps `staked_at` within the weighted-average blend's expected bound; `unstake` before lock maturity correctly fails with `StillLocked` (error 6005 / `0x1775`), and the position is provably untouched afterward. **Not tested: `unstake`'s success path** — see the local-validator entry above for why.
...
- [ ] **`unstake`'s success path has not been tested.** The rejection path (early withdrawal fails correctly) is verified; what happens when a lock actually matures and `unstake` is expected to succeed, move tokens back, and close the account, has not been exercised. Blocked on the local-validator issue above. This is the single most important remaining gap before treating this program as functionally verified.
- [ ] **No test suite has run** in the `anchor test` sense. `tests/act-staking.ts` is a hand-written skeleton, never executed. `scripts/init-devnet-config.mjs` and `scripts/stake-unstake-devnet-test.mjs` cover the happy paths and one failure path (early unstake) manually against live devnet; they aren't a substitute for a suite covering the rest of the failure paths (wrong duration, paused config, zero amount, shortened lock on top-up).
