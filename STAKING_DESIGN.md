...
- [x] ~~**`unstake`'s success path has not been tested.**~~ **Resolved 18 Sept 2026.** Root cause of the local-validator blocker was confirmed (native-Windows `solana-test-validator` genesis-unpacking, a documented upstream issue) and sidestepped, not fixed: a throwaway devnet deployment built with `test-fast-clock` (a "day" = 2 real seconds) proved the success path pays out correctly after maturity, applies no gross-up on the withdrawal-side fee (staker nets `deposit_amount * 0.965^2`, documented and deliberate), and closes the position account with its rent refunded. See `scripts/unstake-success-devnet-test.mjs` and `program/act-staking/README.md`.
- [x] ~~**The lock-shortening fix (ceiling division on top-up) has not been exercised live.**~~ **Resolved 18 Sept 2026** via the same throwaway fast-clock deployment: a fresh position staked at 90 days, a top-up attempting to shorten to 30 days correctly rejected with `CannotShortenLock`, and top-ups to 90 and 365 days both correctly accepted and extended `unlock_at`. See `scripts/lock-shortening-fix-devnet-test.mjs`.
- [x] ~~**No test suite has run** in the `anchor test` sense.~~ **Resolved 18 Sept 2026.** `anchor test`'s local-validator route is still blocked (same root cause as above), so this was solved the same way: `program/act-staking/tests/full-suite.mjs`, a single mocha/chai suite (`npm test`) that builds, deploys, and tears down its own throwaway fast-clock devnet program per run and asserts against every instruction — 16/16 checks passing, covering every failure path the old ad-hoc scripts didn't (wrong duration, paused config, zero amount, shortened lock on top-up, non-authority calls, invalid tier/duration tables, default-pubkey authority handoff). See `program/act-staking/README.md`'s "Automated test suite" section.
- [x] ~~**No independent review**~~ — **Resolved 18 Sept 2026.** Confirmed by the user that the independent review already established for act-presale covers act-staking too.
