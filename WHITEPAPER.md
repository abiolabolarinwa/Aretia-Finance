...
This section matters more than any other section in this document, and a reader pressed for time should read this section even if no other.

1. **Token value can go to zero.** ACT is a freely traded, speculative digital asset. Its market price is determined by open trading and is not supported by any guaranteed value, backing, or redemption right of any kind.
2. **No promise of profit.** Nothing in this document constitutes investment advice or a forecast of price performance. The past performance of any other token, on Solana or elsewhere, is not indicative of anything about ACT's future performance.
3. **Regulatory risk.** Cryptocurrency regulation continues to evolve and varies substantially by jurisdiction. Aretia Finance LLC may need to restrict availability in certain jurisdictions, alter the token's mechanics, or change its own structure in response to legal or regulatory developments, including after any public launch has occurred.
4. **Smart contract and protocol risk.** No independent program review has yet been completed, as discussed in Section 15. On-chain code, including code this project does not itself control, such as the Token-2022 program or the Squads multisig program, can in principle contain undiscovered flaws. Users interact with the protocol at their own risk.
5. **Treasury execution risk.** Multisig custody, described in Section 6, reduces but does not eliminate custodial risk. Vetted project partners may still underperform, misreport, or fail to deliver claimed impact despite the diligence process described in Section 8. The milestone-verification process described in Section 7 is designed to catch such failures before further funds release, not to guarantee they never occur in the first instance.
6. **Liquidity risk.** Locked liquidity, where it exists, reduces but does not eliminate the risk of low trading depth, high price impact, or difficulty exiting a position, particularly in the period shortly after any liquidity pool is first seeded.
7. **Verification risk.** The diligence standards described in Section 8 depend on the continued integrity of third-party registries, engineering certifications, and remote-sensing methodologies that this project does not control and cannot independently guarantee. A registry or certification standard that is itself compromised would propagate that compromise into a project's verified status.
8. **Governance transition risk.** Prior to the activation of Layer 1 governance described in Section 9.3, treasury allocation decisions are made by a team-curated process rather than a token-holder vote. This concentrates a degree of discretion, over which categories and projects are prioritized within the diligence framework of Section 8, in fewer hands than the fully phased-in architecture eventually intends.
9. **This document is a draft.** This expanded revision has not been reviewed by legal counsel, may not accurately describe the final product in all particulars, and may be withdrawn or materially revised at any time before or after publication.

## 15. Limitations and Future Work

A paper organized around the claim of verifiability, as this one is, should state plainly what is not yet verifiable because it does not yet exist. As of the date of this edition, the following remain incomplete.

**Independent program review.** No formal, independent security audit of the mint configuration or of the off-chain scripts that construct treasury transactions has yet been completed, a decision made deliberately given time constraints at initial deployment but flagged in the project's own operational records as worth revisiting before liquidity is seeded at meaningful scale. Section 14, item 4, discloses the resulting risk directly.

**Liquidity.** No liquidity pool has yet been seeded for the ACT-USDT pair discussed in Section 4.3 and Section 12.3. Until it is, ACT is not tradeable on any exchange, and the catalyst fund's replenishment mechanism described in Section 3.2 has no trading volume to act upon, meaning the fund's current balance from trading activity is zero.

**The transparency dashboard.** As discussed in Section 11.2, the dashboard is not yet live. In its absence, the client-side verification tool described in Section 11.3 provides a partial substitute for point-in-time claims, but not the running historical ledger the dashboard is intended to provide.

**Governance activation.** As discussed in Section 9.3, Layers 1 through 4 of the governance architecture in Section 9.2 are not yet active in the form ultimately intended. The treasury presently operates under team-curated grants subject to the same milestone-verification discipline described in Section 7, a narrower guarantee than full layered governance provides.

**A track record.** No project has yet been funded through the process described in Section 7 and Section 8, and no milestone has yet been verified through it. Every mechanism described in this paper has been specified and, where applicable, deployed, but none has yet been exercised against a real climate project. We consider this the most significant limitation of the present edition and do not attempt to minimize it.

**On-chain identity.** As discussed in Section 4.2, the mint cannot acquire on-chain metadata under its current, immutable configuration. This is not a limitation expected to be resolved by future work; it is a permanent consequence of a design choice already made; and it is recorded here for completeness rather than as an item awaiting resolution.

## 16. Conclusion

This paper has described a mechanism that converts a fixed, protocol-enforced share of ordinary token-transfer activity into standing capital for climate mitigation and adaptation work, a catalyst fund that requires no renewed pledge to keep functioning, no trust in any single party's discretion to keep it honest, and no step, from fee withholding through milestone-verified disbursement, that cannot be independently checked against the public ledger by any party willing to do so. The mechanism, as distinct from its eventual scale, is live: ACT is deployed on Solana mainnet with its supply fixed and mint authority revoked, and the treasury multisig operates under tested, exercised, multi-party custody rather than merely configured custody. What remains, catalogued honestly in Section 15 rather than deferred to a future revision, is scale: real trading volume, a seeded liquidity pool, an independent program review, a live transparency dashboard, and a first slate of verified project partners. None of these remaining items requires trusting a new promise to complete; each is a specific, named piece of work whose completion will itself be as verifiable, once done, as everything described in the sections above already is.
