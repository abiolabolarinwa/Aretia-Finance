...
ACT staking program (Aretia Finance, Part VIII Sec. 30 infrastructure). Full design record, tier formula, account layout, and verification history: see [`../../STAKING_DESIGN.md`](../../STAKING_DESIGN.md) at the repo root.

**Compiles clean, is deployed on devnet** at `DpaKPgqdcoY2pQFrHc5xgP4eegbnaCWuRVYFMrrzThtH`, and every instruction — `initialize_config`, `stake` (fresh positions and top-ups), `unstake` (both the early-withdrawal rejection and the success path after a lock matures), `update_tier_config`, `set_paused`, and `set_config_authority` — has been called for real against the live deployment and verified (see `scripts/init-devnet-config.mjs`, `scripts/stake-unstake-devnet-test.mjs`, `scripts/unstake-success-devnet-test.mjs`, `scripts/lock-shortening-fix-devnet-test.mjs`, and `scripts/set-config-authority-devnet.mjs`). `StakeConfig.authority` and this program's upgrade authority both sit on dedicated keypairs, not a shared throwaway key. A full automated test suite now exists and runs clean (`npm test` from this directory; see "Automated test suite" below; 16/16 assertions passing). Independent review has been confirmed to cover this program. See `STAKING_DESIGN.md`'s "Verification history" and "Still open" sections for the full detail.
...
- `scripts/fix-tier-thresholds-devnet.mjs` — sets the tier thresholds to their correct on-chain (base-unit) values via `update_tier_config` (see STAKING_DESIGN.md's tier table for the scaling).
...
### Testing approach: throwaway fast-clock deployments instead of a local validator

To test `unstake`'s success path without waiting 30 real days, `programs/act-staking/src/lib.rs` has a `test-fast-clock` cargo feature that shrinks a "day" to 2 seconds. A build with this feature is deployed only to a throwaway devnet program (never to the primary devnet address or mainnet), exercised, and then closed to reclaim its rent — see `unstake-success-devnet-test.mjs`, `lock-shortening-fix-devnet-test.mjs`, and `tests/full-suite.mjs` above. This environment's own automated test suite uses the same approach, building and tearing down its own throwaway deployment on every run, so this coverage doesn't depend on any particular local-validator setup being available.

Mainnet gate: a real test suite running clean against this devnet deployment, and someone other than the person who wrote it reviewing the program. **Both are now satisfied** — the automated suite (`npm test`, 16/16 passing, see above) and independent review (confirmed to cover this program, same as act-presale). This is still the protocol's first custom on-chain program — see STAKING_DESIGN.md for why that raised the bar relative to everything else in this repo, and its "Still open" section for anything else worth checking before mainnet.
